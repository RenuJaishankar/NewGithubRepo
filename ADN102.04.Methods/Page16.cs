...
namespace Page16
{
    class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            DateTime processFrom;
            if (args.Length == 0)
            {
                processFrom = DateTime.Now;
            }
            else if (args.Length != 1)
            {
                Console.WriteLine("The input is not a valid date, it should be in (mm/dd/yy) format"); return;
            }
            else if (!DateTime.TryParse(args[0], out processFrom))
            {
                Console.WriteLine("The date given is in a invalid Date format");
            }
            ...
            {
                DateTime.TryParse(args[0], out processFrom);
            }

            // in all cases print out a message stating when the late fees are being calculated from
            Console.WriteLine($"The late fees are calculated from: processFrom.ToShortDateString()");
            ...
            while (true)
            {
                ...
                if (datanew.Length != 3)
                {
                    Console.WriteLine("The line{line} of data is invalid, it should have 3 parts separated by commas");
                }
                if (!int.TryParse(datanew[0], out invoiceid))
                {
                    Console.WriteLine($"THe line{line} is not valid ,The first part must be an integer");
                }
                if (!DateTime.TryParse(datanew[1], out invoicedate))
                {
                    Console.WriteLine($"The line{line} is invalid, the second part must be a date ");
                }
                if (!decimal.TryParse(datanew[2], out invoiceamt))
                {
                    Console.WriteLine($"The line{line} is invalid, the third part must be a decimal");
                }
                ...
                Console.WriteLine($"ID : {invoiceid,10} Due: {invoicedate.ToShortDateString(),15} amount:" +$"{invoiceamt,10} Days late {(processFrom - invoicedate).Days,5}Fee :{ComputeLateFee(processFrom,invoicedate,invoiceamt):0.00}");
    ...
    class Invoice
    {
        ...
        public static decimal ComputeLateFee(DateTime ComputeFrom,DateTime DueDate,decimal InvoiceAmount)
        {
            decimal rv = 0;
